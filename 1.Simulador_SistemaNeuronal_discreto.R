...
plot_neural <- function(W) {
  ...
  # Tipo de linha: contínua (1) para peso positivo, pontilhada (2) para peso negativo
  edge_lty <- ifelse(E(graph)$weight > 0, 1, 2)
  
  # Rótulo das arestas: vazio se peso 1 ou -1, senão mostra o valor
  edge_labels <- ifelse(abs(E(graph)$weight) == 1, "", as.character(E(graph)$weight))
  
  # Novo bloco para afastar labels das arestas
  coords <- layout
  ...
  offset <- 0.08  # valor de afastamento (pode ajustar)
  ...
  # Aumentar margem inferior para espaço da legenda
  ...
  # Título
  title("Grafo de peso sináptico da rede neuronal", cex.main = 1.2, line = 1.5, font.main = 1)
  
  # Legenda
  legend("bottomright", 
         legend = c("Conexão excitatória", "Conexão inibitória"),
         lty = c(1, 2), 
         lwd = 2,
         col = "black",
         pt.cex = 1.5,
...
plot_matrix <- function(Matrix) {
  ...
  matrix_data$Value[matrix_data$Value == 1] <- "Conexão excitatória entre os neurônios"
  matrix_data$Value[matrix_data$Value == -1] <- "Conexão inibitória entre os neurônios"
  matrix_data$Value[matrix_data$Value == 0] <- "Sem conexão entre os neurônios"
  
  matrix_data$Value <- factor(matrix_data$Value,
                              levels = c("Conexão excitatória entre os neurônios",
                                         "Conexão inibitória entre os neurônios",
                                         "Sem conexão entre os neurônios"))
  
  # Definir cores
  colors <- c(
    "Conexão excitatória entre os neurônios" = "lightblue",
    "Conexão inibitória entre os neurônios" = "lightpink",
    "Sem conexão entre os neurônios" = "white"
  )
...
plot_phi <- function(u_values) {
  ...
  legend(x = par("usr")[1], y = par("usr")[4], legend = paste("u =", u_values), col = colors, lwd = 2, 
         inset = 0.05, box.lty = 1, box.col = "white", bg = "white", title = "Valores de u",cex = 0.8)
...
plot_redeneuraldisparo_discreto(mod3_1$X)
